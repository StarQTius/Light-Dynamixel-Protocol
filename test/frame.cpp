...
static void frame_DO_receive_a_headerless_frame() {
  using namespace dxl;
  ...
  read_headerless_frame(read, upd::two_complement, output)
...
int main() {
  UNITY_BEGIN();
  RUN_TEST(frame_DO_send_a_frame);
  RUN_TEST(frame_DO_receive_a_headerless_frame);
  return UNITY_END();
}
